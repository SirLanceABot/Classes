...
public class LIDAR_Lite{
	...
	public LIDAR_Lite(I2C.Port port, Address deviceAddress, double samplePeriod)
	{
		...
		if ( mLIDAR.writeBulk(modeConfigurationRegister)) // request LIDAR-Lite configuration
		{
			...
			if ( mLIDAR.readOnly(modeConfiguration, modeConfiguration.length)) // read the LIDAR-Lite configuration
			{
	...
	void Calculate()
	{
		...
		while (IsBusy()) {edu.wpi.first.wpilibj.Timer.delay(.005);} // not expected to be busy here but check to make sure

		/***********acquire distance**********/		//	I2C::WriteBulk() also works
		if ( mLIDAR.write(Register.COMMAND.value, Command.ACQUIRE_DC_CORRECT.value) ) // initiate distance acquisition with DC stabilization
			System.out.printf ( "[LIDAR-Lite] write operation failed line %s\n", Id.__LINE__() );

		while (IsBusy()) {edu.wpi.first.wpilibj.Timer.delay(.005);} // should be busy briefly while acquiring distance
		...
		// mDistance is not changed if the distance read fails - old value is retained; this may not be appropriate
	}
	...
	boolean GetStatus() // get the LIDAR-Lite status byte for others to interpret
	{
		...
		else
		{
			...
			{
				//System.out.printf ( "[LIDAR-Lite] Status Good Reading, status register:%#2x line %s\n", mStatus[0], Id.__LINE__());
				return true;
			}
		}
	...
	boolean IsBusy() // check LIDAR-Lite busy bit on; indicates if Lidar is busy (not ready for another command)
	{
		if ( GetStatus() )	return (mStatus[0] & StatusMask.BUSY.value & 0xFF) != 0; // bit 0 is LIDAR-Lite v2 busy bit
		else return true;
	}

	boolean IsHealthy() // check LIDAR-Lite healthy bit on; indicates if Lidar healthy
	{
		if ( GetStatus() )	return (mStatus[0] & StatusMask.HEALTHY.value & 0xFF) != 0; // bit 5 is LIDAR-Lite v2 healthy bit
		else return false;
	}

	boolean IsGoodReading() // check LIDAR-Lite not-good bits all off and healthy bit on; indicates if distance measurement was valid
	{
		if ( GetStatus() )	return (mStatus[0] & StatusMask.NOT_GOOD_READING.value & 0xFF) == 0 && (mStatus[0] & StatusMask.HEALTHY.value & 0xFF) != 0; // true if no problem bits on and healthy bit on
		else return false;
	}

	/*
		Methods not implemented since they don't make much sense to do so for a LIDAR sensor:
			public int hashCode()
	...
	// register length for reads - all registers are the same length (1) for this LIDAR so don't bother putting that in the enum for each different command
	...
	private static enum Register { // LIDAR internal control registers used in this program
		...
		private Register(int value, int count){
			...
			this.count = count;} // bytes or registers to read in 1 read command
	};
	...
	private static enum Command { // command used in the COMMAND control register 0x00; we only use in this program 1 of the several commands allowed

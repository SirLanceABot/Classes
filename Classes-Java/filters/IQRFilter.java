...
/*
Five Number Summary of a set of data and Interquartile Range (IQR) based outlier replacement
...
4. Box Plot breakpoints (this isn't to encourage Box Plots which many think are confusing since
    visually they depict ranges and not a more expected quantities of points within ranges).
...
Some inspiration for this class was taken from github jbosstm  narayana
*/
...
public class IQRFilter
{
    ...
    /**
     * Median value
     * Assumes data are already sorted
     * @param values
     * @param from
     * @param to
    ...
    static double getMedian(double[] values, int from, int to)
    {
        ...
        if (sz % 2 == 0)
        {
            return (values[from + (sz/2) - 1] + values[from + (sz/2)] ) / 2.;
        }
        else
        {

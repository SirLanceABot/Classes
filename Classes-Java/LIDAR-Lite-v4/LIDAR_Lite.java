...
public class LIDAR_Lite {
	...
	public LIDAR_Lite(I2C.Port port, Address deviceAddress, double samplePeriod) {
		...
		/********** Mode Configuration Register - default is 0x08 **********/
		...
		} else if (mLIDAR.readOnly(modeConfiguration, modeConfiguration.length)) // read the LIDAR-Lite configuration
		{
			...
			int configurationMode = 0x00;  // new value to set
			if (mLIDAR.write(Register.MODE_CONFIGURATION.value, configurationMode))
				System.out.printf("[LIDAR-Lite] write mode configuration failed line %s\n", Id.__LINE__());

				mLIDAR.writeBulk(modeConfigurationRegister);
				mLIDAR.readOnly(modeConfiguration, modeConfiguration.length);
				System.out.printf("[LIDAR-Lite] running with Mode Configuration %#2x\n", modeConfiguration[0]);

				byte highAccuracy[] = new byte[Register.HIGH_ACCURACY_MODE.count];
				byte highAccuracyRegister[] = new byte[RegisterAddressLength];
				highAccuracyRegister[RegisterAddressLength - 1] = (byte) Register.HIGH_ACCURACY_MODE.value;
				int highAccuracyMode = 0x06;  // new value to set
				mLIDAR.write(Register.HIGH_ACCURACY_MODE.value, highAccuracyMode);
		...
		if (IsWorking()) {
			...
			GetAcquisitionCount(); // as found; 0xff default
			int maxAcquisitionCount = 0x18;  // new value to set
 ...
 // the LIDAR is revived somehow
		...
		while (IsBusy()) {
			edu.wpi.first.wpilibj.Timer.delay(.002);
		...
		if (mLIDAR.writeBulk(distanceRegister_1st)) // tell LIDAR-Lite we want to start reading at the 1st distance
			...
			System.out.printf("[LIDAR-Lite] readOnly distance failed line %s\n", Id.__LINE__());
									...
									// int for our use
			mDistance.set(((distance[1] & 0xFF) << 8) | (distance[0] & 0xFF)); // update shared memory with the new
// flipped 0 & 1 for v4 - didn't change the doc below yet																				// LIDAR-Lite value
	...
	private static enum Register { // LIDAR internal control registers used in this program
		...
		ACQUISITION_COUNT(0x05, 1), // read/write the maximum acquisition count 
		DISTANCE_1_2(0x10, 2), // using automatic sequence increment to read both (2) distance registers bytes
		MODE_CONFIGURATION(0xe5, 1), // read the single mode configuration byte
		HIGH_ACCURACY_MODE(0xeb, 1);
...
// From LIDAR-Lite V4 C++ code for Arduino
//  /*------------------------------------------------------------------------------
//   Configure

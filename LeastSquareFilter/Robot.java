...
public class Robot extends TimedRobot {
  ...
  int n = 4; // size of filter window
  double[] y = new double[n]; // y here will be the sensor data history [0] the oldest
  double[] coef = LeastSquaresFiltering(n); 
  
  public void robotInit() {
    for(int k = 0; k < n; k++) {
      y[k] = m_ultrasonic.getValue(); // start the filter history
    }
  }
  ...
  public void teleopPeriodic() {
    ...
    y[n-1] = m_ultrasonic.getValue(); // get the current data
    ...
    SmartDashboard.putNumber("filter diff", y[n-1] - yPredictedSG);
    SmartDashboard.putNumber("actual", y[n-1]);
    SmartDashboard.putNumber("filtered", yPredictedSG);
  }
...
*/
static double[] LeastSquaresFiltering (int n) {
  double[] x = new double[n]; // x here is psuedo time at regularly spaced intervals
  double[] coef = new double[n];
  double xSum=0, xSqrSum=0;

  for (int k = 0; k < n; k++) {
    x[k] = k - (n - 1); // generate the psuedo time; oldest = -(n-1) to newest = 0
    xSum += x[k];
    xSqrSum += x[k]*x[k];
  }
  double xSumSqr = xSum*xSum;
  for (int k = 0; k < n; k++) {
    coef[n-1-k] = (xSqrSum - xSum*x[n-1-k]) / (n*xSqrSum - xSumSqr);
  }
